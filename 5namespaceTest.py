__author__ = 'qcp'
# 测试tf.variable_scope和tf.get_variable
# 利用变量管理方法改进mnist中的inference函数，参数传递通过变量管理完成
# -*- coding:utf-8 -*-
import os
...
with tf.variable_scope('root'):
    ...
    with tf.variable_scope('foo', reuse=True):
        ...
        with tf.variable_scope('bar'):
            print(tf.get_variable_scope().reuse) #输出True
            #当新建一个嵌套的上下文管理器，但不指定reuse，此时reuse取值与外面一层保持一致
    print(tf.get_variable_scope().reuse)# 输出False
    #退出reuse为True的上下文后，reuse又变成False
'''
...
def inference(input_tensor, avg_class, reuse=False):
    ...
    if avg_class == None:
        ...
        with tf.variable_scope('', reuse=True):
            ...
            b1 = tf.get_variable('layer1/biases', [LAYER1_NODE])
            w2 = tf.get_variable('layer2/weights', [LAYER1_NODE, OUTPUT_NODE])
            b2 = tf.get_variable('layer2/biases', [OUTPUT_NODE])
        layer_1 = tf.nn.relu(tf.matmul(input_tensor, avg_class.average(w1)) + avg_class.average(b1))
        return tf.matmul(layer_1, avg_class.average(w2)) + avg_class.average(b2)
